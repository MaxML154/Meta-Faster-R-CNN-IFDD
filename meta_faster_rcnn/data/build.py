...
from meta_faster_rcnn.data.dataset_mapper_ifdd import DatasetMapperWithSupportIFDD
...
def build_detection_train_loader(cfg, mapper=None):
    ...
    if mapper is None:
        if 'ifdd' in cfg.DATASETS.TRAIN[0]:
            mapper = DatasetMapperWithSupportIFDD(cfg, True)
...
def build_detection_test_loader(cfg, dataset_name, mapper=None):
    ...
    if mapper is None:
        if 'ifdd' in dataset_name:
            mapper = DatasetMapperWithSupportIFDD(cfg, False)
        elif 'coco' in cfg.DATASETS.TRAIN[0]:
