...
class FsodRCNN(nn.Module):
    ...
    def init_support_features(self, evaluation_dataset, evaluation_shot, keepclasses, test_seeds):
        ...
        elif self.evaluation_dataset == 'ifdd':
            self.init_model_ifdd()
            
    def init_model_ifdd(self):
        if 1:
            if self.test_seeds == 0:
                if self.keepclasses == 'all':
                    support_path = './datasets/ifdd/metatrain_support_df.pkl'
                else:
                    support_path = './datasets/ifdd/{}_shot_support_df.pkl'.format(self.evaluation_shot)
            elif self.test_seeds > 0:
                if self.keepclasses == 'all':
                    support_path = './datasets/ifdd/seed{}/metatrain_support_df.pkl'.format(self.test_seeds)
                else:
                    support_path = './datasets/ifdd/seed{}/{}_shot_support_df.pkl'.format(self.test_seeds, self.evaluation_shot)

            support_df = pd.read_pickle(support_path)

            metadata = MetadataCatalog.get('ifdd_novel_val')
            # unmap the category mapping ids for IFDD if needed
            ...
            for cls in support_df['category_id'].unique():
                ...
                for index, support_img_df in support_cls_df.iterrows():
                    img_path = os.path.join('./datasets/ifdd', support_img_df['file_path'])
